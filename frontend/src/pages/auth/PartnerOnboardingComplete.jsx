...
export default function PartnerOnboardingComplete() {
  ...
  const onSubmit = async (data) => {
    setLoading(true)
    try {
      // Save/update user profile or onboarding info
      ...
      <div className="max-w-2xl w-full bg-white rounded-3xl shadow-card overflow-hidden border border-gray-100 p-8 sm:p-12 animate-scale-in">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 font-medium max-w-md mx-auto">
            Your Google account is connected. Please fill in your restaurant details to finish setup.
          </p>
        </div>
        ...
        <div className="bg-gradient-to-r from-gray-50 to-slate-50 border border-gray-200/75 rounded-2xl p-4 mb-8 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <h3 className="font-bold text-gray-900 truncate">{user?.name || 'Partner'}</h3>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1" htmlFor="restaurantName">
              Restaurant Name <span className="text-[#FF4F5A]">*</span>
            </label>
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-gray-400">
              ...
              <input
                ...
                className={`block w-full pl-11 pr-4 py-3 bg-gray-50 border ${
                  errors.restaurantName ? 'border-red-500 focus:ring-red-500/20' : 'border-gray-200 focus:border-[#FF4F5A] focus:ring-[#FF4F5A]/20'
                } rounded-xl text-gray-900 placeholder-gray-400 outline-none focus:ring-4 transition-smooth text-[0.95rem]`}
              />
            </div>
            {errors.restaurantName && (
              <p className="mt-1 text-xs text-red-500 font-medium">{errors.restaurantName.message}</p>
            )}
          </div>

          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1" htmlFor="restaurantAddress">
              Restaurant Address <span className="text-[#FF4F5A]">*</span>
            </label>
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-gray-400">
              ...
              <input
                ...
                className={`block w-full pl-11 pr-4 py-3 bg-gray-50 border ${
                  errors.restaurantAddress ? 'border-red-500 focus:ring-red-500/20' : 'border-gray-200 focus:border-[#FF4F5A] focus:ring-[#FF4F5A]/20'
                } rounded-xl text-gray-900 placeholder-gray-400 outline-none focus:ring-4 transition-smooth text-[0.95rem]`}
              />
            </div>
            {errors.restaurantAddress && (
              <p className="mt-1 text-xs text-red-500 font-medium">{errors.restaurantAddress.message}</p>
            )}
          </div>

          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1" htmlFor="businessInfo">
              Business / Cuisine Information
            </label>
            <textarea
              id="businessInfo"
              rows={3}
              placeholder="Cuisine types, operating hours, GST/business license number (optional)..."
              {...register('businessInfo')}
              className="block w-full p-3 bg-gray-50 border border-gray-200 focus:border-[#FF4F5A] focus:ring-[#FF4F5A]/20 rounded-xl text-gray-900 placeholder-gray-400 outline-none focus:ring-4 transition-smooth text-[0.95rem]"
            />
          </div>
